    ...
    fetch(`./src/${query}.html`)
      ...
      .then(html => {
        ...
        if (previous_article != undefined){
          document.getElementById("Button_previous").textContent = `前の記事：${data["articles"][previous_article]["name"]}`;
        ...
        if (next_article != undefined){
          document.getElementById("Button_next").textContent = `次の記事：${data["articles"][next_article]["name"]}`;
